          ...
          <div className="d-flex flex-column justify-content-center align-items-center" style={{ flex: 1, padding: '20px' }}>
            ...
            <form onSubmit={handleSubmit} style={{ width: '100%', maxWidth: '600px' }}>
              <div className="input-group border rounded overflow-hidden">
                <input type="text" className="form-control border-0" placeholder="Type your message here..." value={userPrompt} onChange={(e) => setUserPrompt(e.target.value)} required style={{ backgroundColor: '#2c2c2c', color: '#fff', padding: '10px', borderRadius: '5px 0 0 5px', border: '1px solid #444' }} />
                <button className="btn" type="submit" style={{ backgroundColor: '#ff9900', color: '#fff', border: 'none', padding: '10px 20px', borderRadius: '0 5px 5px 0' }}>Generate</button>
              </div>
            </form>
          </div>
        ) : (
          <div className="d-flex flex-column justify-content-between" style={{ height: '100vh', position: 'relative', backgroundColor: '#000', color: '#fff' }}>
            <div ref={chatContainerRef} className="container" style={{ maxHeight: '80vh', overflowY: 'auto', marginBottom: '80px', padding: '10px', display: 'flex', flexDirection: 'column' }}>
              {chatMessages.map((message) => (
                <div key={message.id} style={{ backgroundColor: message.type === 'user' ? '#ff9900' : '#444', color: '#fff', borderRadius: '10px', padding: '10px', marginBottom: '10px', maxWidth: '80%', wordWrap: 'break-word', whiteSpace: 'pre-wrap', alignSelf: message.type === 'user' ? 'flex-end' : 'flex-start' }}>
                  <span>{message.content}</span>
                  {message.type === 'ai' && message.showSendButton && (
                    <div className="mt-2">
                      <button className="btn btn-sm" style={{ backgroundColor: '#b8860b', color: '#fff' }} onClick={handleSend}>Send</button>
                    </div>
                  )}
                </div>
              ))}
